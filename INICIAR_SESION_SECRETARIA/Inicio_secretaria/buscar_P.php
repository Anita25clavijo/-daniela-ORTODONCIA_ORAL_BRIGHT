...
<body>
    ...
    ?>
    <form action="buscar_P.php" method="GET" class="form_search">
    ...
    <tr>
        ...
        <?php 
    //paginador
    $sql_register=mysqli_query($conexion,"SELECT COUNT(*) as total_registro from paciente where(Id_Paciente like'%$busqueda%' or Nombre_Paciente like '%$busqueda%' or Apellido_Paciente like '%$busqueda%' or Direccion_Paciente like'%$busqueda%' or Telefono_Paciente like '%$busqueda%' or Correo_Paciente like '%$busqueda%' or Fecha_Nacimiento like '%$busqueda%' or Estado_Paciente like '%$busqueda%')and Estado_Paciente!='INACTIVO' ");
...
$total_paginas= ceil($total_registro / $x_pag);
         ...
         $resultado = mysqli_query($conexion,"SELECT * FROM paciente where(Id_Paciente like'%$busqueda%' or Nombre_Paciente like '%$busqueda%' or Apellido_Paciente like '%$busqueda%' or Direccion_Paciente like'%$busqueda%' or Telefono_Paciente like '%$busqueda%' or Correo_Paciente like '%$busqueda%' or Fecha_Nacimiento like '%$busqueda%' or Estado_Paciente like '%$busqueda%')and Estado_Paciente!='INACTIVO' order by Id_Paciente LIMIT $desde,$x_pag");
         while($data=mysqli_fetch_assoc($resultado)) { ?>
        
    <tr>
        
    <th><?php echo $data ["Id_Paciente"];?></th>    
    <th><?php echo $data ["Nombre_Paciente"];?></th>
    <th><?php echo $data ["Apellido_Paciente"];?></th>
    <th><?php echo $data ["Direccion_Paciente"];?></th>    
    <th><?php echo $data ["Telefono_Paciente"];?></th>
    <th><?php echo $data ["Correo_Paciente"];?></th>
    <th><?php echo $data ["Fecha_Nacimiento"];?></th>    
    <th><?php echo $data ["Estado_Paciente"];?></th>
    <th>
        <a href="Modificar_p.php?Id= <?php echo $data["Id_Paciente"];?>" class="button green" >Modificar</a> 
        <a href="eliminarp.php?Id= <?php echo $data ["Id_Paciente"]?>" class="button red" >Eliminar</a>
    </th>
