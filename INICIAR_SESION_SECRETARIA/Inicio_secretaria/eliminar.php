...
if(!empty($_POST)){
    ...
    if($query_delete=true){
     echo "<script>alert('Se han Eliminado correctamente, actualice para ver los cambios'); window.location='http://localhost/(daniela)ORTODONCIA_ORAL_BRIGHT/INICIAR_SESION_SECRETARIA/Inicio_secretaria/ActualizarU.php';</script>";
} else {
...
{
    header("location:ActualizarU.php");
    
}else{
    $conexion=mysqli_connect("localhost","root","","mydb");
    mysqli_set_charset ($conexion,"utf8");
    $idusuario = $_REQUEST['id'];
    $query= mysqli_query($conexion,"SELECT u.Nombre_Usuario, u.Contraseña, r.descripcion FROM usuario as u INNER JOIN tipo_usuario as r ON u.id_tipo_usuario= r.id 
                                    WHERE u.Id_Usuario = $idusuario ");
    $resultado=mysqli_num_rows($query);
    if($resultado > 0){
        while($data= mysqli_fetch_array($query)){
            
            $nombreu= $data['Nombre_Usuario'];
            $contra= $data['Contraseña'];
            $rol= $data['descripcion'];
        }
    }else{
        header("locatiob: ActualizarU.php");
    }
}
...
<body>
 ...
 <h2>¿Esta seguro de eliminar el sigueinte usuario?</h2>
                <p>Nombre: <span><?php echo $nombreu; ?></span> </p>
                <p>Contraseña: <span><?php echo $contra; ?></span> </p>
                <p>Rol: <span><?php echo $rol; ?></span> </p>
                
         <input type="hidden" name="Id" value="<?php echo $idusuario;?>">
        <input  type="submit"  class="button green" value="Aceptar">
        <a href="ActualizarU.php" type="submit"  class="button red">Cancelar</a>
