...
<body style="height:30px">
    ...
    <br>
    <div>
    <form action="buscar_A.php" method="GET" class="form_search">
    ...
    <tr>
        ...
        <?php 
    //paginador
    $sql_register=mysqli_query($conexion,"SELECT COUNT(*) as total_registro from agenda where Estado_Agenda!='INACTIVO' ");
...
$total_paginas= ceil($total_registro / $x_pag);
         ...
         $resultado = mysqli_query($conexion,"SELECT a.*, p.* ,m.* , h.* FROM agenda a inner join paciente p inner join medico m inner join historia_clinica h where a.Id_Paciente=p.Id_Paciente and a.Id_Medico=m.Id_Medico and a.id_historia_clinica=h.Id_Historia and a.Estado_Agenda!='INACTIVO' order by Id_Agenda LIMIT $desde,$x_pag");
    ...
    <th>
        <a href="ModificarA.php?Id= <?php echo $data["Id_Agenda"];?>" class="button green" >Modificar</a> 
        <a href="EliminarA.php?Id= <?php echo $data ["Id_Agenda"]?>" class="button red" >Eliminar</a>
    </th>
...
<center style="height:15px;"><a href="agregarA.php"  class="button" style="margin-top:20px;">Agregar</a></center>
<div class="paginador">
       <ul>
           <?php 
           if($pagina != 1){
