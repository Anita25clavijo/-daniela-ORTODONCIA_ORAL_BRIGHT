...
if(!empty($_POST)){
 ...
 if($query_registro=true){
 	echo "<script>alert('Se han actualizado los cambios correctamente, actualice la pagina para ver los cambios'); window.location='http://localhost/(daniela)ORTODONCIA_ORAL_BRIGHT/INICIO_SESION/INICIAR_SESION_DOCTOR/GestionA.php';</script>";
 }
 else{
 	echo"<script>alert('No se registro con exito'); window.history.go(-1); </script>";
		...
		echo "<script>alert('Ya esta ocupado este turno'); window.location='http://localhost/(daniela)ORTODONCIA_ORAL_BRIGHT/INICIO_SESION/INICIAR_SESION_DOCTOR/GestionA.php';</script>";
	}
